...
contract Escrow {
    address public nftAddress; //collection of nft addreess
    ...
    function cancelSale(uint256 _nftID) public onlyBuyerOrSeller(_nftID) {
        ...
        if (!inspectionPassed[_nftID]) {
            uint256 refund = escrowAmount[_nftID];
            payable(buyer[_nftID]).transfer(refund);
        } else {
            uint256 sellerAmount = address(this).balance;
