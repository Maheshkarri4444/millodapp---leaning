...
describe('Escrow', () => {
    ...
    it('Refunds buyer if inspection failed', async () => {
        // Inspector sets inspection to false
    ...
    it('Pays seller if inspection passed', async () => {
        ...
        // Seller should receive funds (ignoring gas costs)
        expect(balanceAfter).to.be.above(balanceBefore);
        expect(await escrow.isListed(1)).to.equal(false);
    });

    it('Reverts if called by non-buyer/seller', async () => {
        await expect(
            escrow.connect(lender).cancelSale(1)
        ).to.be.revertedWith("Not both buyer or seller");
    });
});
